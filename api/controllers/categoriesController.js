...
const find = async function(req, res, next){
    try {
        let result = await categoriesService.find(req.query);
...
const findById = async function(req, res, next){
    try {
        let result = await categoriesService.findById(req.params.index);
        res.status(200).send(utilResponse.format(result, "get"));
    } catch (error) {
        res.statusCode = error.errors ? 400 : 500;
        res.send(utilResponse.format(error));
...
const create = async function(req, res, next){
    try {
        let result = await categoriesService.create(req.userJwt, req.body);
...
const update = async function(req, res, next){
    try {
        let result = await categoriesService.update(req.userJwt, req.params.index, req.body);
...
const destroy = async function(req, res, next){
    try {
        let result = await categoriesService.destroy(req.userJwt, req.params.index, req.body);
